...
public class MoneyBags extends Player {
	private ArrayList<Double> percents;
	int baseBet;
	HandEval eval;
	
	public MoneyBags(int startingChips){
		super(startingChips);
	...
	public int yourBet(){
		...
		if(hand.handSize() == 2){
			...
			if(rankOfHand.getRank() < 4){
				return 0;
			}
			yourbet = baseBet * rankOfHand.getRank()*2;
			if(getChips()<=yourbet ){
				return getChips();
			}
			return yourbet;
		...
		while (x < percents.size()){
			yourbet = (int) (baseBet * percents.get(x) * x*2);
			x++;
		}
